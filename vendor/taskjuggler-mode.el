...
(defun taskjuggler-insert-existing-task ()
  "Insert an existing task at point."
  (interactive)
  (insert (taskjuggler-select-task)))

(defun taskjuggler-copy-task ()
  "Place a task in the kill ring."
  (interactive)
  (kill-new (taskjuggler-select-task)))

(defun taskjuggler-select-task ()
  "Select at task."
  (interactive)
...
(defvar taskjuggler-command "tj3"
  "The taskjuggler command-line utility to call")

;; Compile
(defun taskjuggler-build-compile-command (buffer &optional args)
  (concat taskjuggler-command
          " "
          (cond
...
(defun taskjuggler-check-syntax ()
  (interactive)
  (let ((cmd (taskjuggler-build-compile-command (current-buffer) "--no-color --silent --check-syntax")))
    (compile cmd)))

;; (TODO) check if flycheck is available first!
(flycheck-define-checker taskjuggler-tj3
  "A taskjuggler syntax checker."
  :command ("tj3" "--no-color" "--silent" "--check-syntax"
            source)
  :standard-input 0
  :error-patterns
...
(add-to-list 'flycheck-checkers 'taskjuggler-tj3)
